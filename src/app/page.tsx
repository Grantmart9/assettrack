/**
 * Home page component (Dashboard) for the AssetTrack application.
 * This is the main landing page after authentication, showing asset overview,
 * overdue inspections, quick actions, and recent assets table.
 *
 * Features:
 * - Unauthenticated view with login/signup prompts
 * - Loading spinner during data fetch
 * - Dashboard cards: total assets, overdue inspections, quick actions
 * - Recent assets table with status badges
 * - Error handling for data fetching
 *
 * Uses React hooks for state management and Supabase services for data.
 */
...
/**
 * Main HomePage component - renders the dashboard or auth prompt.
 * Fetches assets and overdue inspections on mount.
 * Handles unauthenticated, loading, and error states.
 *
 * @returns {JSX.Element} The home page JSX, either auth prompt or dashboard.
 */
export default function HomePage() {
   ...
   * Calls assetService.getAll() and inspectionService.getOverdueInspections().
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch overdue inspections from the service
        const { data: overdueData, error: overdueError } =
          await inspectionService.getOverdueInspections();
        if (overdueError) {
          setError(
            "Error fetching overdue inspections: " + overdueError.message
          );
          return;
        }
  ...
  /**
   * Render unauthenticated view with login/signup buttons.
   * Centered layout prompting user to authenticate.
   */
  if (!user) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="max-w-md w-full text-center">
          ...
          <p className="text-xl text-gray-600 mb-8">
            Track and manage your assets efficiently
          </p>
   ...
   * Simple centered spinner for user feedback during async operations.
   */
  if (loading) {
    return (
  ...
  // Main dashboard render - authenticated user view
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
        <div className="px-4 py-6 sm:px-0">
          ...
          {/* Dashboard metric cards grid - responsive layout */}
          ...
          {/* Recent Assets Section - table showing last 5 assets */}
            ...
            <div className="px-4 py-5 sm:px-6">
              {assets.length > 0 ? (
                <div className="overflow-x-auto">
                  {/* Responsive table for recent assets */}
                  <table className="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {/* Map over first 5 assets, render rows with status badges */}
